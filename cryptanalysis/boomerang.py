...
def computeFeistelBoomerangDifferential(cipher, parameters):
    ...
    while not search.reachedTimelimit(start_time, parameters["timelimit"]):
        prob = feistelBoomerangTrailSearch(
...
def feistelBoomerangTrailSearch(cipher, parameters, timestamp, boomerangProb=0):
    ...
    # Store optimal weight found for upper trail
    # the new round of search of E0 will start from this weight if the first attempt failed(w=99)
    ...
    if parameters["lweight"] < parameters["wordsize"] / parameters["sboxSize"]:
        searchLimit = (
            parameters["wordsize"] / parameters["sboxSize"] - parameters["lweight"]
        )
    else:
    ...
    while (
        ...
        and lowerWeight < parameters["lweight"] + searchLimit
    ):

        # Search Lower Trail
        lowerCharacteristic = boomerangTrail(
            cipher, parameters, timestamp, "lower", beta
        )
        ...
        try:
            ...
            parameters["blockedLowerCharacteristics"].clear()
            return 0
        lowerWeight = parameters[
            "sweight"
        ]  # Store optimal weight found for lower trail
        ...
        if switchProb != 0:
            ...
            if "X0" not in parameters["boomerangVariables"]:
                ...
                print(
                    "Fixed X0 in boomerang to {}".format(
                        parameters["boomerangVariables"]["X0"]
                    )
                )
                print("----")
            ...
            if (
                ...
                not in parameters["boomerangVariables"]
            ):
                parameters["boomerangVariables"][
                    "X{}".format(parameters["lowertrail"])
                ] = delta
                print(
                    "Fixed X{} in boomerang to {}".format(
                        parameters["lowertrail"],
                        parameters["boomerangVariables"][
                            "X{}".format(parameters["lowertrail"])
                        ],
            ...
            while (
                ...
                diff_upper = boomerangDifferential(
                    cipher, parameters, alpha, beta, upperWeight, timestamp, "upper"
                )
            diff_lower = 0
            while (
                not search.reachedTimelimit(start_time, parameters["timelimit"])
                ...
                diff_lower = boomerangDifferential(
                    cipher, parameters, gamma, delta, lowerWeight, timestamp, "lower"
                )
            ...
            boomerangProb += (
                diff_upper * diff_upper * diff_lower * diff_lower * switchProb
            )
            print(
                "Found boomerang trail: {}, {}, {}".format(
                    math.log(diff_upper, 2),
                    math.log(diff_lower, 2),
    ...
    print(
        "Completed trail search with boomerang probability of {}".format(
            math.log(boomerangProb, 2)
        )
    )
...
def boomerangTrail(
    ...
    while (
        ...
        if fixedPoint in parameters["boomerangVariables"]:
            parameters["fixedVariables"][fixedPoint] = parameters["boomerangVariables"][
                fixedPoint
            ]
            print(
                "Fixed {} to {}".format(
                    fixedPoint, parameters["fixedVariables"][fixedPoint]
                )
            )
                ...
                if input != 0:
                    ...
                    for output in range(16):
                        ...
                        if (
                            "X{}".format(parameters["lowertrail"])
                            not in parameters["boomerangVariables"]
                            and parameters["bct"][input][output] != 16
                        ):
                            a = "X0[{0}:{1}]".format(
                                (parameters["perm"][n] * 4) + 3,
                                parameters["perm"][n] * 4,
