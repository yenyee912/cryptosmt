...
class SPARXRoundCipher(AbstractCipher):
    ...
    def createSTP(self, stp_filename, parameters):
        ...
        with open(stp_filename, "w") as stp_file:
            ...
            #     stp_file, upperEndRound, switchRound, lowerStartRound, 0x0, 0x2, part
            # )
    ...
    def setupSwitchConstraints(
        ...
        if a == 0x0 and b == 0x2:
            self.zero2switch(
                stp_file, parameters, upperEndRound, switchRound, lowerStartRound, part
            )
            ...
            self.two2switch(stp_file, upperEndRound, switchRound, lowerStartRound, part)

    def zero2switch(
        self, stp_file, parameters, upperEndRound, switchRound, lowerStartRound, part
    ):
        """
        ...
        if part == "upper":
            stp_file.write(
                ...
                # f"ASSERT(NOT((Y0{upperEndRound} | Y1{upperEndRound}) = 0b0000000000000000));\n"
                # f"ASSERT(NOT((X16) = 0x8E0E));\n"
            )

            print(parameters["blockedLowerCharacteristics"])

        elif part == "lower":
            print("hellooo")
            if switchRound % self.rounds_per_step == 0:
                """
                if switch= r3/6/9, means e1 start from round X03/6/9, but hard to constraint X03 reversely
                therefore need to target X0A2, and let e1 start from X03
                """
                stp_file.write(
                    # f"ASSERT((X0A2 & 0b0000000000000010) =  ((BVXOR(X0A2, X1A2) & 0b0000000000001000)));\n"
                    # f"ASSERT((Y0A2 & 0b0000000000000010) =  ((BVXOR(Y0A2, Y1A2) & 0b0000000000001000)));\n"
                    # f"ASSERT(NOT((X0A2 | X1A2) = 0b0000000000000000));\n"
                    # f"ASSERT(NOT((Y0A2 | Y1A2) = 0b0000000000000000));\n"
                    # f"ASSERT((X00 & 0b0000000000000010) =  ((BVXOR(X00, X10) & 0b0000000000001000)));\n"
                    f"ASSERT((Y00 & 0b0000000000000010) =  ((BVXOR(Y00, Y10) & 0b0000000000001000)));\n"
                    # f"ASSERT(NOT((Y00 | Y10) = 0b0000000000000000));\n"
                )
            elif (switchRound - 2) % self.rounds_per_step == 0:
                """
                if switch= r2/5/8 = 2-2/5-2/8-2, means e1 start from X02/X05/X08,
                because X03>>(ky)>>X04>>(ky-ABCT)>>X05 is EQUAL to X00 >>(ky)>>X01>>(ky)>>X02
                so skip round need to skip r0>>r1, and r1>>(abct)>>r2
                should have set params.skipround to 2, but 2 and 1 have same effect so just stick with 1
                """
                stp_file.write(
                    # f"ASSERT((X02 & 0b0000000000000010) =  ((BVXOR(X02, X12) & 0b0000000000001000)));\n"
                    # f"ASSERT((Y02 & 0b0000000000000010) =  ((BVXOR(Y02, Y12) & 0b0000000000001000)));\n"
                    f"ASSERT(NOT((X02 | X12) = 0b0000000000000000));\n"
                    f"ASSERT(NOT((X02) = 0x50C1));\n"
                    f"ASSERT(NOT((X12) = 0x8020));\n"
                )

            else:
                """
                if switch= r4/7 = means e1 start from X04/7,
                because X03>>(ky-ABCT)>>X04>>(ky)>>X05 == X00 >>(ky-abct)>>X01>>(ky)>>X02
                so skip round need to skip r0>>(abct)>>r1 only (less one round than previous)
                should have set params.skipround to 2, but 2 and 1 have same effect so just stick with 1
                """
                stp_file.write(
                    # f"ASSERT((X01 & 0b0000000000000010) =  ((BVXOR(X01, X11) & 0b0000000000001000)));\n"
                    f"ASSERT((Y01 & 0b0000000000000010) =  ((BVXOR(Y01, Y11) & 0b0000000000001000)));\n"
                    f"ASSERT(NOT((Y01 | Y11) = 0b0000000000000000));\n"
                    # f"ASSERT(NOT(X01 = 0x2800));\n"
                    # f"ASSERT(NOT(X11 = 0x0010));\n"
                )

        else:
            print("Please specify the part of search: upper (E0) or lower (E1).")

    def zero1switch(self, stp_file, upperEndRound, switchRound, lowerStartRound, part):
        ...
        if part == "upper":
            ...
            if (switchRound) % self.rounds_per_step == 0:
                ...
                stp_file.write(
                    f"ASSERT((X0A2 & 0b0000000000000100) =  ((BVXOR(X0A2, X1A2) & 0b0000000000000100))));\n"
                    f"ASSERT((Y0A2 & 0b0000000000000100) =  ((BVXOR(Y0A2, Y1A2) & 0b0000000000000100)));\n"
                    f"ASSERT(NOT((X0A2 | X1A2) = 0b0000000000000000));\n"
                    f"ASSERT(NOT((Y0A2 | Y1A2) = 0b0000000000000000));\n"
                )
            elif (switchRound - 2) % self.rounds_per_step == 0:
                stp_file.write(
                    f"ASSERT((X02 & 0b0000000000000100) =  ((BVXOR(X02, X12) & 0b0000000000000100)));\n"
                    f"ASSERT((Y02 & 0b0000000000000100) =  ((BVXOR(Y02, Y12) & 0b0000000000000100)));\n"
                    f"ASSERT(NOT((X02 | X12) = 0b0000000000000000));\n"
                )

            else:
                stp_file.write(
                    f"ASSERT((X01 & 0b0000000000000001) =  ((BVXOR(X01, X11) & 0b0000000000000100)));\n"
                    f"ASSERT((Y01 & 0b0000000000000001) =  ((BVXOR(Y01, Y11) & 0b0000000000000100)));\n"
                    f"ASSERT(NOT((X01 | X11) = 0b0000000000000000));\n"
                )
        else:
            print("Please specify the part of search: upper (E0) or lower (E1).")

    def two2switch(self, stp_file, upperEndRound, switchRound, lowerStartRound, part):
        """
        ...
        if part == "upper":
            ...
            if (switchRound) % self.rounds_per_step == 0:
                # because of the skip round so we can start at x0a2--> actually weight start from wl3 wr3
                stp_file.write(
                    f"ASSERT(NOT(BVXOR((X0A2 & 0b0000000000000011), (BVXOR(X0A2, X1A2) & 0b0000000000000100)) = 0b0000000000000010));\n"
                    f"ASSERT(NOT(BVXOR((Y0A2 & 0b0000000000000011), (BVXOR(Y0A2, Y1A2) & 0b0000000000000100)) = 0b0000000000000010));\n"
                    f"ASSERT(NOT((X0A2 | X1A2) = 0b0000000000000000));\n"
                    # f"ASSERT(NOT((Y0A2 | Y1A2) = 0b0000000000000000));\n"
                )
            elif (switchRound - 2) % self.rounds_per_step == 0:
                ...
                stp_file.write(
                    f"ASSERT(NOT(BVXOR((X00 & 0b0000000000000011), ((BVXOR(X00, X10) &0b0000000000000100)) = 0b0000000000000010));\n"
                    f"ASSERT(NOT(BVXOR((X00 & 0b0000000000000011), ((BVXOR(X00, X10) &0b0000000000000100)) = 0b0000000000000010));\n"
    ...
    def setupFixedSwitchConstraints(
        self, stp_file, upperEndRound, switchRound, lowerStartRound, part
    ):
        """
        ...
        if part == "lower":
            stp_file.write(
                # f"ASSERT((X00 & 0b0000000000010100) =  (X10 & 0b0000000001010000));\n"
                f"ASSERT((X0A2 & 0b0000000000010100) =  ((BVXOR(X0A2, X1A2) & 0b0000000001010000)));\n"
                # f"ASSERT(NOT(( X00 | X10 ) = 0b0000000000000000));\n"
                f"ASSERT(NOT(( X0A2 | X1A2 ) = 0b0000000000000000));\n"
            )
